...
@section('title', config('app.name') . ' - Edit Mohalla')
@section('page_title', 'Edit Mohalla')
@section('breadcrumb', 'Edit Mohalla')
            ...
            <div class="white_card_header">
                <div class="box_header m-0">
                    <div class="main-title">
                        <h3 class="m-0">Edit Mohalla</h3>
                    </div>
                </div>
            </div>
            <div class="white_card_body">
                <form action="{{ route('mohallas.update', $mohalla) }}" method="POST">
                    @csrf
                    @method('PUT')
                                            ...
                                            <select name="village_id" id="village_id" class="form-control" required>
                                                ...
                                                @foreach($villages as $village)
                                                    <option value="{{ $village->id }}" 
                                                            ...
                                                            {{ old('village_id', $mohalla->village_id) == $village->id ? 'selected' : '' }}>
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="text" name="name" class="form-control" value="{{ old('name', $mohalla->name) }}" required>
                            @error('name')
                                <div class="text-danger">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="col-md-6 mb-3">
                            <div class="form-check mt-4">
                                <input class="form-check-input" type="checkbox" name="is_active" id="is_active" value="1" {{ old('is_active', $mohalla->is_active) ? 'checked' : '' }}>
...
<script>
    $(document).ready(function() {
        ...
        // Set initial filter values based on selected village
        @if($mohalla->village_id)
            @php
                $selectedVillage = $villages->firstWhere('id', $mohalla->village_id);
            @endphp
            @if($selectedVillage && $selectedVillage->unionCouncil)
                @if($selectedVillage->unionCouncil->tehsil)
                    $('#filterDistrict').val('{{ $selectedVillage->unionCouncil->tehsil->district_id ?? '' }}').trigger('change');
                    setTimeout(function() {
                        $('#filterTehsil').val('{{ $selectedVillage->unionCouncil->tehsil_id ?? '' }}').trigger('change');
                        setTimeout(function() {
                            $('#filterUnionCouncil').val('{{ $selectedVillage->union_council_id ?? '' }}');
                        }, 100);
                    }, 100);
                @endif
            @endif
