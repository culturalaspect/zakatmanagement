...
@section('title', config('app.name') . ' - Add Installment')
@section('page_title', 'Add Installment')
@section('breadcrumb', 'Add Installment')
            ...
            <div class="white_card_header">
                <div class="box_header m-0">
                    <div class="main-title">
                        <h3 class="m-0">Add Installment to Fund Allocation</h3>
                        <p class="text-muted mb-0">Financial Year: {{ $fundAllocation->financialYear->name }} | Total Amount: Rs. {{ number_format($fundAllocation->total_amount, 2) }}</p>
                    </div>
                </div>
            </div>
            <div class="white_card_body">
                <form action="{{ route('installments.store', $fundAllocation) }}" method="POST" id="installmentForm">
                    @csrf
                    ...
                    <div class="row">
                        <div class="col-md-4 mb-3">
                            ...
                            <input type="number" name="installment_number" class="form-control" value="{{ old('installment_number', $nextInstallmentNumber) }}" min="1" required>
                        ...
                        <div class="col-md-4 mb-3">
                            ...
                            <input type="number" name="installment_amount" id="installment_amount" class="form-control" value="{{ old('installment_amount') }}" step="0.01" min="0" max="{{ $remainingAmount }}" required>
                            <small class="text-muted">
                                Allocated Fund: Rs. {{ number_format($fundAllocation->total_amount, 2) }}
                    ...
                    <div class="row">
                        <div class="col-12">
                            ...
                            <a href="{{ route('fund-allocations.show', $fundAllocation) }}" class="btn btn-secondary">Cancel</a>
                        </div>
                    </div>
                </form>
            </div>
...
<script>
    $(document).ready(function() {
        ...
        const allocatedFund = parseFloat({{ $fundAllocation->total_amount }}) || 0;
        ...
        // Repopulate form with old input if validation failed
        ...
        function updateSchemePreview(quotaIndex) {
            ...
            $(`.scheme-remaining-percentage[data-quota-index="${quotaIndex}"]`).text(remainingPercentage.toFixed(2));
            $(`.scheme-total-beneficiaries[data-quota-index="${quotaIndex}"]`).text(totalBeneficiaries.toFixed(1));
            $(`.scheme-total-amount[data-quota-index="${quotaIndex}"]`).text('Rs. ' + totalAmount.toLocaleString('en-US', {minimumFractionDigits: 2, maximumFractionDigits: 2}));
