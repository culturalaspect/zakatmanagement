...
@section('title', config('app.name') . ' - Edit Local Zakat Committee')
@section('page_title', 'Edit Local Zakat Committee')
@section('breadcrumb', 'Edit Local Zakat Committee')
            ...
            <div class="white_card_header">
                <div class="box_header m-0">
                    <div class="main-title">
                        <h3 class="m-0">Edit Local Zakat Committee</h3>
                        <p class="text-muted mb-0">Code: <strong>{{ $localZakatCommittee->code ?? 'N/A' }}</strong></p>
                    </div>
                </div>
            </div>
            <div class="white_card_body">
                <form action="{{ route('local-zakat-committees.update', $localZakatCommittee) }}" method="POST" id="committeeForm">
                    @csrf
                    @method('PUT')
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <select name="district_id" id="district_id" class="form-control" required>
                                ...
                                <option value="{{ $district->id }}" {{ old('district_id', $localZakatCommittee->district_id) == $district->id ? 'selected' : '' }} data-district-name="{{ $district->name }}">
                                    {{ $district->name }}
                                </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="text" name="name" class="form-control" value="{{ old('name', $localZakatCommittee->name) }}" required>
                            @error('name')
                                <div class="text-danger">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="col-md-12 mb-3">
                            ...
                            <textarea name="area_coverage" class="form-control" rows="3">{{ old('area_coverage', $localZakatCommittee->area_coverage) }}</textarea>
                            @error('area_coverage')
                                <div class="text-danger">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="col-md-4 mb-3">
                            ...
                            <input type="date" name="formation_date" class="form-control" value="{{ old('formation_date', $localZakatCommittee->formation_date ? $localZakatCommittee->formation_date->format('Y-m-d') : '') }}" required>
                        ...
                        <div class="col-md-4 mb-3">
                            ...
                            <input type="number" name="tenure_years" class="form-control" value="{{ old('tenure_years', $localZakatCommittee->tenure_years) }}" min="1" required>
                        ...
                        <div class="col-md-4 mb-3">
                            <div class="form-check mt-4">
                                <input class="form-check-input" type="checkbox" name="is_active" id="is_active" value="1" {{ old('is_active', $localZakatCommittee->is_active) ? 'checked' : '' }}>
                    ...
                    <div class="mb-3">
                        ...
                        @php
                            $selectedMohallaIds = old('mohalla_ids', $localZakatCommittee->mohallas->pluck('id')->toArray());
                        @endphp
                        <select name="mohalla_ids[]" id="mohalla_ids" class="form-control" multiple="multiple" required>
                            ...
                            <option value="{{ $mohalla->id }}" 
                                    ...
                                    {{ in_array($mohalla->id, $selectedMohallaIds) ? 'selected' : '' }}>
                    ...
                    <div class="row">
                        <div class="col-12">
                            ...
                            <a href="{{ route('local-zakat-committees.show', $localZakatCommittee) }}" class="btn btn-secondary">Cancel</a>
                        </div>
                    </div>
                </form>
            </div>
...
<script>
    $(document).ready(function() {
        ...
        @php
            $selectedMohallaIds = old('mohalla_ids', $localZakatCommittee->mohallas->pluck('id')->toArray());
        @endphp
        selectedValues = @json($selectedMohallaIds);
