...
@section('title', config('app.name') . ' - View Local Zakat Committee')
@section('page_title', 'View Local Zakat Committee')
@section('breadcrumb', 'View Local Zakat Committee')
            ...
            <div class="white_card_header">
                <div class="box_header m-0">
                    ...
                    <div class="header_more_tool">
                        <a href="{{ route('local-zakat-committees.edit', $localZakatCommittee) }}" class="btn btn-primary">
            ...
            <div class="white_card_body">
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        <p><strong class="text-primary">{{ $localZakatCommittee->code ?? 'N/A' }}</strong></p>
                    </div>
                    <div class="col-md-6 mb-3">
                        <strong>Committee Name:</strong>
                        <p>{{ $localZakatCommittee->name }}</p>
                    </div>
                    <div class="col-md-6 mb-3">
                        <strong>District:</strong>
                        <p>{{ $localZakatCommittee->district->name ?? 'N/A' }}</p>
                    </div>
                    <div class="col-md-12 mb-3">
                        <strong>Area Coverage:</strong>
                        <p>{{ $localZakatCommittee->area_coverage ?? 'N/A' }}</p>
                    </div>
                    <div class="col-md-4 mb-3">
                        <strong>Formation Date:</strong>
                        <p>{{ $localZakatCommittee->formation_date ? \Carbon\Carbon::parse($localZakatCommittee->formation_date)->format('d M Y') : 'N/A' }}</p>
                    </div>
                    <div class="col-md-4 mb-3">
                        <strong>Tenure (Years):</strong>
                        <p>{{ $localZakatCommittee->tenure_years ?? 'N/A' }}</p>
                    </div>
                    <div class="col-md-4 mb-3">
                        <strong>Tenure End Date:</strong>
                        <p>{{ $localZakatCommittee->tenure_end_date ? \Carbon\Carbon::parse($localZakatCommittee->tenure_end_date)->format('d M Y') : 'N/A' }}</p>
                    </div>
                    <div class="col-md-6 mb-3">
                        <strong>Status:</strong>
                        <p>
                            @if($localZakatCommittee->is_active)
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <p><span id="totalMembersCount">{{ $localZakatCommittee->members->count() ?? 0 }}</span></p>
                    </div>
                </div>

                <hr class="my-4">
                <div class="d-flex justify-content-between align-items-center mb-3">
                    <h5 class="mb-0">Serving Mohallas (<span id="mohallasCount">{{ $localZakatCommittee->mohallas->count() }}</span>)</h5>
                ...
                <div id="mohallasTableContainer">
                    @if($localZakatCommittee->mohallas->count() > 0)
                    <div class="table-responsive">
                        <table class="table table-sm" id="mohallasTable">
                            ...
                            <tbody id="mohallasTableBody">
                                @foreach($localZakatCommittee->mohallas as $mohalla)
                ...
                <div id="membersTableContainer">
                    @if($localZakatCommittee->members->count() > 0)
                    <!-- Filters Section -->
                    <div class="row mb-4">
                        <div class="col-12">
                                    ...
                                    <div class="row">
                                        <div class="col-md-3 mb-3">
                                            ...
                                            <select id="filterGender" class="form-control form-control-sm">
                                                ...
                                                @foreach($uniqueGenders as $gender)
                                                    <option value="{{ ucfirst($gender) }}">{{ ucfirst($gender) }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                        <div class="col-md-3 mb-3">
                                        ...
                                        <div class="col-md-3 mb-3">
                                            ...
                                            <select id="filterVerificationStatus" class="form-control form-control-sm">
                                                ...
                                                @foreach($uniqueVerificationStatuses as $status)
                    ...
                    <div class="table-responsive">
                        <table class="table" id="membersTable">
                            ...
                            <tbody id="membersTableBody">
                                @foreach($localZakatCommittee->members as $member)
                                <tr data-member-id="{{ $member->id }}">
                                    <td><strong>{{ $member->cnic }}</strong></td>
                                    ...
                                    <td>
                                        <div class="action_btns d-flex">
                                            <button type="button" class="action_btn mr_10 viewMemberBtn" data-member-id="{{ $member->id }}" title="View">
                                                <i class="ti-eye"></i>
                                            </button>
                                            @if($member->verification_status == 'pending')
                                            <button type="button" class="action_btn mr_10 verifyMemberBtn" data-member-id="{{ $member->id }}" title="Verify">
                                                <i class="ti-check"></i>
                                            </button>
                                            @endif
                                            @if($member->verification_status != 'verified')
                                            <button type="button" class="action_btn mr_10 editMemberBtn" data-member-id="{{ $member->id }}" data-verification-status="{{ $member->verification_status }}" title="Edit">
                                                <i class="ti-pencil"></i>
                                            </button>
                                            @else
                ...
                <div class="row mt-4">
                    <div class="col-12">
                        <a href="{{ route('local-zakat-committees.index') }}" class="btn btn-secondary">Back to List</a>
                    </div>
                </div>
            </div>
        </div>
...
<script>
    $(document).ready(function() {
        const committeeId = {{ $localZakatCommittee->id }};
        let availableMohallas = @json($availableMohallas ?? []);
        ...
        if ($('#membersTable').length) {
            membersTable = $('#membersTable').DataTable({
                ...
                buttons: [
                    ...
                    },
                    {
                        ...
                        className: 'btn btn-sm btn-secondary',
                        filename: 'committee-' + committeeId + '-members-' + new Date().toISOString().split('T')[0],
                        exportOptions: {
            ...
            $('#filterSearch').on('keyup', function() {
                membersTable.search(this.value).draw();
            });

            // Clear filters button
            $('#clearFilters').on('click', function() {
                $('#filterGender').val('').trigger('change');
                $('#filterVerificationStatus').val('').trigger('change');
        ...
        $('#addMohallaBtn').on('click', function() {
            ...
            const tehsils = @json($tehsils ?? []);
            const unionCouncils = @json($unionCouncils ?? []);
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: '{{ route("local-zakat-committees.add-mohalla", $localZakatCommittee) }}',
                        method: 'POST',
                        data: {
                            _token: '{{ csrf_token() }}',
        ...
        $(document).on('click', '.deleteMohallaBtn', function() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: '{{ route("local-zakat-committees.remove-mohalla", $localZakatCommittee) }}',
                        method: 'POST',
                        data: {
                            _token: '{{ csrf_token() }}',
        ...
        $('#addMemberBtn').on('click', function() {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: '{{ route("lzc-members.store-ajax") }}',
                        method: 'POST',
                        data: {
                            _token: '{{ csrf_token() }}',
                            ...result.value
