...
@section('title', config('app.name') . ' - Edit Phase')
@section('page_title', 'Edit Phase')
@section('breadcrumb', 'Edit Phase')
    ...
    <div class="col-lg-12">
        <div class="white_card mb_30">
            ...
            <div class="white_card_body">
                <form action="{{ route('phases.update', $phase) }}" method="POST" id="phaseForm">
                    @csrf
                    @method('PUT')
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <select name="installment_id" id="installment_id" class="form-control" required>
                                ...
                                @foreach($installments as $installment)
                                    <option value="{{ $installment->id }}" {{ old('installment_id', $phase->installment_id) == $installment->id ? 'selected' : '' }}
                                        data-financial-year="{{ $installment->fundAllocation->financialYear->name ?? '' }}">
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="number" name="phase_number" id="phase_number" class="form-control" value="{{ old('phase_number', $phase->phase_number) }}" min="1" required>
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="text" name="name" id="phase_name" class="form-control" value="{{ old('name', $phase->name) }}" placeholder="Will be auto-generated">
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="number" name="max_beneficiaries" id="max_beneficiaries" class="form-control" value="{{ old('max_beneficiaries', $phase->max_beneficiaries) }}" min="1" required>
                        ...
                        <div class="col-md-6 mb-3" id="max_amount_div" style="display: none;">
                            ...
                            <input type="number" name="max_amount" id="max_amount" class="form-control" value="{{ old('max_amount', $phase->max_amount) }}" min="0" step="0.01">
                            @error('max_amount')
                                <div class="text-danger">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="date" name="start_date" class="form-control" value="{{ old('start_date', $phase->start_date ? $phase->start_date->format('Y-m-d') : '') }}" required>
                            @error('start_date')
                                <div class="text-danger">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="date" name="end_date" class="form-control" value="{{ old('end_date', $phase->end_date ? $phase->end_date->format('Y-m-d') : '') }}">
                            @error('end_date')
                                <div class="text-danger">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="col-md-6 mb-3">
                            ...
                            <select name="status" id="status" class="form-control" required>
                                <option value="open" {{ old('status', $phase->status) == 'open' ? 'selected' : '' }}>Open</option>
                                <option value="closed" {{ old('status', $phase->status) == 'closed' ? 'selected' : '' }}>Closed</option>
                            </select>
                            @error('status')
                                <div class="text-danger">{{ $message }}</div>
                            @enderror
...
<script>
    $(document).ready(function() {
        ...
        const currentMaxBeneficiaries = {{ $phase->max_beneficiaries }};
        const currentPhaseSchemeId = {{ $phase->scheme_id ?? 0 }};
        ...
        function loadQuotaInfo(selectSchemeId = null) {
            ...
            const schemeId = selectSchemeId || $('#scheme_id').val();
        ...
        function displayQuotaInfo(data) {
            ...
            // Calculate current max amount based on scheme type
            ...
            if (schemeSelect.val() && currentMaxBeneficiaries > 0) {
                if (isLumpSum) {
                    // For lump sum, use the current phase's max_amount value
                    currentMaxAmount = parseFloat('{{ $phase->max_amount ?? 0 }}') || 0;
                } else {
                    // For non-lump sum, calculate based on current beneficiaries
        ...
        let lastGeneratedName = '{{ old('name', $phase->name) }}';
        ...
        if ($('#installment_id').val()) {
            const currentDistrictId = {{ $phase->district_id }};
