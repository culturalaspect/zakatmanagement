...
@section('title', config('app.name') . ' - Edit Union Council')
@section('page_title', 'Edit Union Council')
@section('breadcrumb', 'Edit Union Council')
            ...
            <div class="white_card_header">
                <div class="box_header m-0">
                    <div class="main-title">
                        <h3 class="m-0">Edit Union Council</h3>
                    </div>
                </div>
            </div>
            <div class="white_card_body">
                <form action="{{ route('union-councils.update', $unionCouncil) }}" method="POST">
                    @csrf
                    @method('PUT')
                                            ...
                                            <select name="tehsil_id" id="tehsil_id" class="form-control" required>
                                                ...
                                                @foreach($tehsils as $tehsil)
                                                    <option value="{{ $tehsil->id }}" 
                                                            ...
                                                            {{ old('tehsil_id', $unionCouncil->tehsil_id) == $tehsil->id ? 'selected' : '' }}>
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="text" name="name" class="form-control" value="{{ old('name', $unionCouncil->name) }}" required>
                            @error('name')
                                <div class="text-danger">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="col-md-6 mb-3">
                            <div class="form-check mt-4">
                                <input class="form-check-input" type="checkbox" name="is_active" id="is_active" value="1" {{ old('is_active', $unionCouncil->is_active) ? 'checked' : '' }}>
...
<script>
    $(document).ready(function() {
        ...
        // Set initial filter value based on selected tehsil
        @if($unionCouncil->tehsil_id)
            @php
                $selectedTehsil = $tehsils->firstWhere('id', $unionCouncil->tehsil_id);
            @endphp
            @if($selectedTehsil)
                $('#filterDistrict').val('{{ $selectedTehsil->district_id ?? '' }}');
            @endif
        @endif
