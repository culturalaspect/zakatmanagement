                    ...
                    <div class="table-responsive" style="overflow-x: auto;">
                        <table id="phasesTable" class="table table-hover" style="width:100%; min-width: 1200px;">
                            ...
                            <tbody>
                                ...
                                <tr style="cursor: pointer;" onclick="window.location.href='{{ route('beneficiary-applications.phase', $phase->id) }}'">
                                    <td>
                                        <strong>{{ $phase->name }}</strong>
                                    </td>
                                    <td>
                                        {{ $phase->installment->fundAllocation->financialYear->name ?? 'N/A' }}
                                    </td>
                                    <td>
                                        Installment #{{ $phase->installment->installment_number ?? 'N/A' }}
                                    </td>
                                    <td>{{ $phase->district->name ?? 'N/A' }}</td>
                                    <td>{{ $phase->scheme->name ?? 'N/A' }}</td>
                                    <td>{{ $phase->start_date ? $phase->start_date->format('d M Y') : 'N/A' }}</td>
                                    <td>{{ $phase->end_date ? $phase->end_date->format('d M Y') : 'N/A' }}</td>
                                    <td>
                                        <span class="badge bg-info">
                                            {{ number_format($phase->current_beneficiaries_count) }} / {{ number_format($phase->max_beneficiaries) }}
                                        </span>
                                        @if($phase->remaining_beneficiaries > 0)
                                            <br><small class="text-success">Remaining: {{ number_format($phase->remaining_beneficiaries) }}</small>
                                        @else
                                    ...
                                    <td>
                                        <span class="badge bg-info">
                                            Rs. {{ number_format($phase->current_amount, 2) }} / Rs. {{ number_format($phase->max_amount, 2) }}
                                        </span>
                                        @if($phase->remaining_amount > 0)
                                            <br><small class="text-success">Remaining: Rs. {{ number_format($phase->remaining_amount, 2) }}</small>
                                        @else
                                            <br><small class="text-danger">Limit Reached</small>
                                        @endif
                                    </td>
                                    ...
                                    <td>
                                        <a href="{{ route('beneficiary-applications.phase', $phase->id) }}" 
