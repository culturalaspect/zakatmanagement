...
@section('title', config('app.name') . ' - Edit Village')
@section('page_title', 'Edit Village')
@section('breadcrumb', 'Edit Village')
                ...
                <form action="{{ route('villages.update', $village) }}" method="POST">
                                            ...
                                            <select name="union_council_id" id="union_council_id" class="form-control" required>
                                                ...
                                                @foreach($unionCouncils as $uc)
                                                    <option value="{{ $uc->id }}" 
                                                            ...
                                                            {{ old('union_council_id', $village->union_council_id) == $uc->id ? 'selected' : '' }}>
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="text" name="name" class="form-control" value="{{ old('name', $village->name) }}" required>
                            @error('name')
                                <div class="text-danger">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="col-md-6 mb-3">
                            <div class="form-check mt-4">
                                <input class="form-check-input" type="checkbox" name="is_active" id="is_active" value="1" {{ old('is_active', $village->is_active) ? 'checked' : '' }}>
...
<script>
    $(document).ready(function() {
        ...
        // Set initial filter values based on selected union council
        @if($village->union_council_id)
            @php
                $selectedUnionCouncil = $unionCouncils->firstWhere('id', $village->union_council_id);
            @endphp
            @if($selectedUnionCouncil && $selectedUnionCouncil->tehsil)
                $('#filterDistrict').val('{{ $selectedUnionCouncil->tehsil->district_id ?? '' }}').trigger('change');
                setTimeout(function() {
                    $('#filterTehsil').val('{{ $selectedUnionCouncil->tehsil_id ?? '' }}');
                }, 100);
            @endif
        @endif
