...
@section('title', config('app.name') . ' - District-wise Report')
@section('page_title', 'District-wise Report')
@section('breadcrumb', 'Reports / District-wise')
...
<div class="row">
    <div class="col-12">
        <div class="card">
            <div class="card-body">
                <form method="get" action="{{ route('reports.district-wise') }}" class="row g-3 mb-4">
                    <div class="col-md-3">
                ...
                <div class="table-responsive">
                    <table class="table table-bordered table-striped">
                        ...
                        <tbody>
                            @forelse($districtStats as $name => $stat)
                                <tr>
                                    <td>{{ $name }}</td>
                        ...
                        <tfoot class="table-light">
                            <tr>
                                <th>Total</th>
                                <th class="text-end">{{ number_format($districtStats->sum(fn($s) => $s['count'])) }}</th>
                                <th class="text-end">{{ number_format($districtStats->sum(fn($s) => $s['amount']), 2) }}</th>
                            </tr>
                        </tfoot>
                        @endif
                    </table>
