...
<head>
    <meta charset="UTF-8">
    <title>Institution-wise Report</title>
...
<body>
    <div class="header"><h1>Institution-wise Report</h1><p>Generated: {{ date('d M Y H:i') }}</p></div>
    <table>
        <thead><tr><th>Institution</th><th>Beneficiaries</th><th>Total Amount (Rs.)</th></tr></thead>
        <tbody>
            @foreach($instStats as $name => $stat)
                <tr><td>{{ $name }}</td><td>{{ number_format($stat['count']) }}</td><td>{{ number_format($stat['amount'], 2) }}</td></tr>
            @endforeach
        </tbody>
        @if($instStats->isNotEmpty())
        <tfoot><tr><th>Total</th><th>{{ number_format($instStats->sum(fn($s) => $s['count'])) }}</th><th>{{ number_format($instStats->sum(fn($s) => $s['amount']), 2) }}</th></tr></tfoot>
        @endif
    </table>
