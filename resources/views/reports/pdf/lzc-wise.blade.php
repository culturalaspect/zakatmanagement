...
<body>
    <div class="header"><h1>LZC / Committee-wise Report</h1><p>Generated: {{ date('d M Y H:i') }}</p></div>
    <table>
        <thead><tr><th>Local Zakat Committee</th><th>Beneficiaries</th><th>Total Amount (Rs.)</th></tr></thead>
        <tbody>
            @foreach($lzcStats as $name => $stat)
                <tr><td>{{ $name }}</td><td>{{ number_format($stat['count']) }}</td><td>{{ number_format($stat['amount'], 2) }}</td></tr>
            @endforeach
        </tbody>
        @if($lzcStats->isNotEmpty())
        <tfoot><tr><th>Total</th><th>{{ number_format($lzcStats->sum(fn($s) => $s['count'])) }}</th><th>{{ number_format($lzcStats->sum(fn($s) => $s['amount']), 2) }}</th></tr></tfoot>
        @endif
    </table>
