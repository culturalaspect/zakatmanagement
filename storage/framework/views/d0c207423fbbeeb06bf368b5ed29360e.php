...
<?php $__env->startSection('title', config('app.name') . ' - Villages'); ?>
<?php $__env->startSection('page_title', 'Villages'); ?>
<?php $__env->startSection('breadcrumb', 'Villages'); ?>
            ...
            <div class="white_card_header">
                <div class="box_header m-0">
                    <div class="main-title">
                        <h3 class="m-0">Villages</h3>
                    </div>
                    <div class="header_more_tool">
                        <a href="<?php echo e(route('villages.create')); ?>" class="btn btn-primary">
                            <i class="ti-plus"></i> Add Village
                        </a>
                    </div>
                </div>
            </div>
            <div class="white_card_body">
                <?php if($villages->isEmpty()): ?>
                    <div class="alert alert-info">
                        <i class="ti-info-alt"></i> No villages found.
                    </div>
                <?php else: ?>
                    <!-- Filters Section -->
                    <div class="row mb-4">
                                        ...
                                        <div class="col-md-3 mb-3">
                                            ...
                                            <select id="filterUnionCouncil" class="form-control form-control-sm">
                                                ...
                                                <?php $__currentLoopData = $unionCouncils; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $uc): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                                    <option value="<?php echo e($uc->name); ?>" data-tehsil-id="<?php echo e($uc->tehsil_id); ?>"><?php echo e($uc->name); ?> (<?php echo e($uc->tehsil->district->name ?? ''); ?>)</option>
                                                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                                            </select>
                                        </div>
                                        <div class="col-md-3 mb-3">
                        ...
                        <table class="table" id="villagesTable">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td data-union-council="<?php echo e($village->unionCouncil->name ?? ''); ?>"><?php echo e($village->unionCouncil->name ?? 'N/A'); ?></td>
                                    <td data-tehsil="<?php echo e($village->unionCouncil->tehsil->name ?? ''); ?>"><?php echo e($village->unionCouncil->tehsil->name ?? 'N/A'); ?></td>
                                    <td data-district="<?php echo e($village->unionCouncil->tehsil->district->name ?? ''); ?>"><?php echo e($village->unionCouncil->tehsil->district->name ?? 'N/A'); ?></td>
                                    <td><?php echo e($village->mohallas->count() ?? 0); ?></td>
                                    <td data-status="<?php echo e($village->is_active ? 'active' : 'inactive'); ?>">
                                    ...
                                    <td>
                                        <div class="action_btns d-flex">
                                            <a href="<?php echo e(route('villages.show', $village)); ?>" class="action_btn mr_10" title="View">
                                                <i class="ti-eye"></i>
                                            </a>
                                            <a href="<?php echo e(route('villages.edit', $village)); ?>" class="action_btn mr_10" title="Edit">
                                                <i class="ti-pencil"></i>
                                            </a>
                                            <form action="<?php echo e(route('villages.destroy', $village)); ?>" method="POST" class="d-inline mr_10" onsubmit="return confirmDelete(event, 'Are you sure you want to delete this village?')">
                                                <?php echo csrf_field(); ?>
                                ...
                                <tr>
                                    <td colspan="7" class="text-center">No villages found</td>
                                </tr>
                                <?php endif; ?>
                            </tbody>
                        </table>
...
<script>
    $(document).ready(function() {
        var table = $('#villagesTable').DataTable({
            ...
            buttons: [
                ...
                },
                {
                    ...
                    className: 'btn btn-sm btn-secondary',
                    filename: 'villages-' + new Date().toISOString().split('T')[0],
                    exportOptions: {
                        columns: [0, 1, 2, 3, 4, 5],
                        format: {
                            body: function(data, row, column, node) {
        ...
        var districtFilter = function(settings, data, dataIndex) {
            ...
            var row = table.row(dataIndex).node();
            var rowDistrict = $(row).find('td:eq(3)').attr('data-district');
        ...
        var tehsilFilter = function(settings, data, dataIndex) {
            ...
            var row = table.row(dataIndex).node();
            var rowTehsil = $(row).find('td:eq(2)').attr('data-tehsil');
        ...
        var unionCouncilFilter = function(settings, data, dataIndex) {
            ...
            var row = table.row(dataIndex).node();
            var rowUC = $(row).find('td:eq(1)').attr('data-union-council');
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\php_projects_new\zakat_beneficiaries\laravel_project\resources\views/villages/index.blade.php ENDPATH**/ ?>