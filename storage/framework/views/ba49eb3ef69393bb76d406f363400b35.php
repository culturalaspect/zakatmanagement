<?php $__env->startSection('title', config('app.name') . ' - Users'); ?>
                        ...
                        <table class="table" id="usersTable">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td data-role="<?php echo e($user->role); ?>">
                                        <?php if($user->role === 'super_admin'): ?>
                                            <span class="badge bg-danger">Super Admin</span>
                                        <?php elseif($user->role === 'administrator_hq'): ?>
                                            <span class="badge bg-warning">Administrator HQ</span>
                                        <?php elseif($user->role === 'district_user'): ?>
                                            <span class="badge bg-info">District User</span>
                                        <?php else: ?>
                                            <span class="badge bg-secondary"><?php echo e($user->role); ?></span>
                                        <?php endif; ?>
                                    </td>
                                    <td><?php echo e($user->district ? $user->district->name : 'N/A'); ?></td>
                                <td><?php echo e($user->created_at->format('Y-m-d H:i')); ?></td>
                                <td>
                                    <div class="action_btns d-flex">
                                        <a href="<?php echo e(route('users.show', $user)); ?>" class="action_btn mr_10" title="View">
                                            <i class="ti-eye"></i>
                                        </a>
                                        <?php if(!(auth()->user()->isAdministratorHQ() && !auth()->user()->isSuperAdmin() && $user->role === 'super_admin')): ?>
                                            <a href="<?php echo e(route('users.edit', $user)); ?>" class="action_btn mr_10" title="Edit">
                                                <i class="ti-pencil"></i>
                                            </a>
                                        <?php endif; ?>
                                        <?php if($user->id !== auth()->id() && !(auth()->user()->isAdministratorHQ() && !auth()->user()->isSuperAdmin() && $user->role === 'super_admin')): ?>
                                        <form action="<?php echo e(route('users.destroy', $user)); ?>" method="POST" class="d-inline mr_10" onsubmit="return confirmDelete(event, 'Are you sure you want to delete this user?')">
                                            <?php echo csrf_field(); ?>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\php_projects_new\zakat_beneficiaries\laravel_project\resources\views/users/index.blade.php ENDPATH**/ ?>